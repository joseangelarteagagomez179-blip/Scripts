...
-- === CREAR INTERFAZ — DISEÑO CUADRADO ESQUINAS REDONDEADAS ===
...
CreatorLabel.TextTransparency = 0.4 -- Transparente
...
ContentContainer.CornerRadius = UDim.new(0, 10)
...
local function loadInfoTab()
    ...
    updateBox.CornerRadius = UDim.new(0, 6)
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        -- Auto Fuel — llevar a RescueZone
        if autoFuelEnabled then
            local zone = workspace:FindFirstChild("RescueZone")
            if zone then
                for _, v in ipairs(workspace:GetChildren()) do
                    if v:IsA("BasePart") and selectedFuel and string.find(v.Name:lower(), selectedFuel:lower()) then
...
print("[✅] Script cargado correctamente — 99 Noches en el Bosque v1.1")
